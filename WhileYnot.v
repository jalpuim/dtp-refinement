...
Definition setIdent (ident: ptr) (n : nat) : heap -> heap :=
  fun h => match (h # ident) with
            | None => h
            | Some v => h ## ident <- (Axioms.Dyn n, frac v)
           end.
...
Definition getIdent (ident: ptr) (h : heap) : nat.
  refine (match (h # ident) with
            | None => 0
            | Some v => _ (* val v *)
          end).
  destruct v.
